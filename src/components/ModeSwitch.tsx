...
const ModeSwitch = ({ mode, onModeChange }: ModeSwitchProps) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="relative">
        ...
        >
          ...
          <button
            ...
            className={`relative px-5 py-3 md:px-7 md:py-3.5 rounded-full transition-all duration-500 ease-out ${mode === 'institutional'
                ? 'text-background'
          ...
          >
            ...
            <span className={`relative z-10 font-display text-xs md:text-sm tracking-[0.15em] uppercase font-medium ${isPressed && mode !== 'institutional' ? 'scale-95' : ''
          ...
          <button
            ...
            onTouchStart={() => setIsPressed(true)}
            onTouchEnd={() => setIsPressed(false)}
            className={`relative px-5 py-3 md:px-7 md:py-3.5 rounded-full transition-all duration-500 ease-out ${mode === 'creator'
                ? 'text-background'
                : 'text-foreground/40 hover:text-foreground/60 active:scale-95'
              }`}
          >
            ...
            <span className={`relative z-10 font-display text-xs md:text-sm tracking-[0.15em] uppercase font-medium ${isPressed && mode !== 'creator' ? 'scale-95' : ''
              } transition-transform duration-150`}>
              Creator
            </span>
          </button>
      ...
      {/* Mode indicator text */}
      <p className={`mt-6 text-[10px] md:text-xs font-mono tracking-[0.2em] uppercase transition-colors duration-500 ${mode === 'institutional' ? 'text-institutional/40' : 'text-creator/40'
        }`}>
